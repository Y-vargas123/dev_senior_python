...
def agregar_Expermiento():
    ...
    resultados =list(map(int,input("Ingrese los resultados obtenidos separados por coma: ").split(",")))
...
def calcular_estadisticas():
    ...
    if (0 <= opcion<len(listaExperimentos)):
     promedios = sum(listaExperimentos[opcion][3])/len(listaExperimentos[opcion][3])
     maximos = max(listaExperimentos[opcion][3])
     minimos = min(listaExperimentos[opcion][3])

    print(f"Promedio general de todos los experimentos: {promedios}")
    print(f"Máximo general de todos los experimentos: {maximos}")
    print(f"Mínimo general de todos los experimentos: {minimos}")
